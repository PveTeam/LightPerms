...
public static class LocalizationHelper
{
    public static Dictionary<string, string> GetTexts(MyLanguagesEnum language, Dictionary<MyLanguagesEnum, Dictionary<string, string>> translations, Logging log = null)
    {
        ...
        if (language == MyLanguagesEnum.English || !translations.TryGetValue(language, out Dictionary<string, string> requestedTranslation)) requestedTranslation = null;

        foreach (var kv in fallbackTranslation)
        {
            if (requestedTranslation == null || !requestedTranslation.TryGetValue(kv.Key, out var translation))
            {
    ...
    public static MyStringId GetStringId(Dictionary<string, string> texts, string key)
    {
        ...
        {
            throw new($"GetStringId Failed for Key={key}", ex);
        }
    }
