...
using IMyAttachableTopBlock = Sandbox.ModAPI.IMyAttachableTopBlock;
...
using IMyShipWelder = IMyShipWelder;
using IMyTerminalBlock = IMyTerminalBlock;
using MyInventoryItem = MyInventoryItem;
...
public class NanobotBuildAndRepairSystemBlock : MyGameLogicComponent
{
    ...
    public static readonly MyDefinitionId ElectricityId = new(typeof(MyObjectBuilder_GasProperties), "Electricity");
    ...
    private static readonly MySoundPair[] _Sounds =
        { null, null, null, new("ToolLrgWeldMetal"), new("BlockModuleProductivity"), new("BaRUnable"), new("ToolLrgGrindMetal"), new("BlockModuleProductivity"), new("BaRUnable"), new("BaRUnable") };

    private static readonly float[] _SoundLevels = { 0f, 0f, 0f, 1f, 0.5f, 0.4f, 1f, 0.5f, 0.4f, 0.4f };

    private static readonly int MaxTransportEffects = 50;
    private static int _ActiveTransportEffects;
    private static readonly int MaxWorkingEffects = 80;
    private static int _ActiveWorkingEffects;

    private readonly Stopwatch _DelayWatch = new();
    private readonly HashSet<IMyInventory> _Ignore4Components = [];
    private readonly HashSet<IMyInventory> _Ignore4Ingot = [];
    private readonly HashSet<IMyInventory> _Ignore4Items = [];
    private readonly List<IMyInventory> _PossibleSources = [];

    private readonly HashSet<IMyInventory> _TempIgnore4Components = [];
    private readonly HashSet<IMyInventory> _TempIgnore4Ingot = [];
    private readonly HashSet<IMyInventory> _TempIgnore4Items = [];
    private readonly Dictionary<string, int> _TempMissingComponents = new();
    private readonly HashSet<TargetEntityData> _TempPossibleFloatingTargets = [];
    private readonly HashSet<TargetBlockData> _TempPossibleGrindTargets = [];
    private readonly HashSet<IMyInventory> _TempPossibleSources = [];
    private readonly HashSet<TargetBlockData> _TempPossibleWeldTargets = [];
    private readonly CacheList<MyEntity> _tempEntities = new();

    private bool _AsyncUpdateSourcesAndTargetsRunning;
    ...
    private MyInventory _TransportInventory;
    ...
    internal SyncBlockSettings Settings => _Settings ?? (_Settings = SyncBlockSettings.Load(this, NanobotBuildAndRepairSystemMod.ModGuid, BlockWeldPriority, BlockGrindPriority, ComponentCollectPriority));

    internal NanobotBuildAndRepairSystemBlockPriorityHandling BlockWeldPriority { get; } = [];

    internal NanobotBuildAndRepairSystemBlockPriorityHandling BlockGrindPriority { get; } = [];

    internal NanobotBuildAndRepairSystemComponentPriorityHandling ComponentCollectPriority { get; } = [];

    public IMyShipWelder Welder { get; private set; }

    public SyncBlockState State { get; } = new();

    public Dictionary<IMySlimBlock, TimeSpan> FriendlyDamage => _FriendlyDamage ?? (_FriendlyDamage = new());
    ...
    public override void Init(MyObjectBuilder_EntityBase objectBuilder)
    {
        ...
        Welder = Entity as IMyShipWelder;
        ...
        if (Settings == null) //Force load of settings (is much faster here than initial load in UpdateBeforeSimulation10_100)
            if (Mod.Log.ShouldLog(Logging.Level.Error))
                Mod.Log.Write(Logging.Level.Error, "BuildAndRepairSystemBlock {0}: Initializing Load-Settings failed", Logging.BlockName(Welder, Logging.BlockNameOptions.None));
        ;
        ...
        if (Mod.Log.ShouldLog(Logging.Level.Event)) Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Initialized", Logging.BlockName(Welder, Logging.BlockNameOptions.None));
    }
    ...
    public void SettingsChanged()
    {
        ...
        var resourceSink = Welder.Components.Get<MyResourceSinkComponent>();
        ...
        if (Mod.Log.ShouldLog(Logging.Level.Event))
            Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Init Inventory Volume {1}/{2} MaxTransportVolume={3} Mode={4}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                          (float)Welder.GetInventory(0).MaxVolume, _TransportInventory.MaxVolume, _MaxTransportVolume, Settings.SearchMode);
    }
    ...
    {
        if (_IsInit) return;
        if (Welder.SlimBlock.IsProjected() || !Welder.Synchronized) //Synchronized = !IsPreview
        {
            if (Mod.Log.ShouldLog(Logging.Level.Event)) Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Init Block is only projected/preview -> exit", Logging.BlockName(Welder, Logging.BlockNameOptions.None));
        ...
        Welder.EnabledChanged += block => { UpdateCustomInfo(true); };
        Welder.IsWorkingChanged += block => { UpdateCustomInfo(true); };

        var welderInventory = Welder.GetInventory(0);
        if (welderInventory == null) return;
        _TransportInventory = new MyInventory((float)welderInventory.MaxVolume / MyAPIGateway.Session.BlocksInventorySizeMultiplier, Vector3.MaxValue, MyInventoryFlags.CanSend);
        ...
        if (Mod.Log.ShouldLog(Logging.Level.Event))
            Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Init Block TransportInventory Added to welder MaxVolume {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                          (float)welderInventory.MaxVolume / MyAPIGateway.Session.BlocksInventorySizeMultiplier);
        SettingsChanged();
        ...
        _SoundVolumeSet = -1;
        _IsInit = true;
        if (Mod.Log.ShouldLog(Logging.Level.Event)) Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Init -> done", Logging.BlockName(Welder, Logging.BlockNameOptions.None));
    }
    ...
    {
        if (Mod.Log.ShouldLog(Logging.Level.Event)) Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Close", Logging.BlockName(Welder, Logging.BlockNameOptions.None));
        if (_IsInit)
        {
            ServerEmptyTranportInventory(true);
            Settings.Save(Entity, NanobotBuildAndRepairSystemMod.ModGuid);
            if (Mod.Log.ShouldLog(Logging.Level.Event)) Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Close Saved Settings {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Settings.GetAsXML());
            lock (NanobotBuildAndRepairSystemMod.BuildAndRepairSystems)
            {
    ...
    public override void UpdateBeforeSimulation()
    {
        try
        {
            ...
            if (Welder == null || !_IsInit) return;

            if (!MyAPIGateway.Utilities.IsDedicated)
            {
                if ((Settings.Flags & SyncBlockSettings.Settings.ShowArea) != 0)
                {
                    var colorWelder = Welder.SlimBlock.GetColorMask().HSVtoColor();
                    ...
                    var emitterMatrix = Welder.WorldMatrix;
                    emitterMatrix.Translation = Vector3D.Transform(Settings.CorrectedAreaOffset, emitterMatrix);
                    MySimpleObjectDraw.DrawTransparentBox(ref emitterMatrix, ref areaBoundingBox, ref color, MySimpleObjectRasterizer.Solid, 1, 0.04f, RangeGridResourceId);
                }
        ...
        {
            if (Mod.Log.ShouldLog(Logging.Level.Error)) Mod.Log.Write(Logging.Level.Error, "BuildAndRepairSystemBlock {0}: UpdateBeforeSimulation Exception:{1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), ex);
        }
    }
    ...
    {
        if (Mod.Log.ShouldLog(Logging.Level.Event)) Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: UpdatingStopped", Logging.BlockName(Welder, Logging.BlockNameOptions.None));
        if (_IsInit)
            Settings.Save(Entity, NanobotBuildAndRepairSystemMod.ModGuid);
        //Stop sound effects
        StopSoundEffects();
    ...
    private void UpdateBeforeSimulation10_100(bool fast)
    {
        try
        {
            ...
            if (_DelayWatch.ElapsedMilliseconds > 40)
            {
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Event))
                    Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: Delay {1} ({2}ms)", Logging.BlockName(Welder, Logging.BlockNameOptions.None), _Delay, _DelayWatch.ElapsedMilliseconds);
            }
        }
        catch (Exception ex)
        {
            if (Mod.Log.ShouldLog(Logging.Level.Error)) Mod.Log.Write(Logging.Level.Error, "BuildAndRepairSystemBlock {0}: UpdateBeforeSimulation10/100 Exception:{1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), ex);
        }
    }
    ...
    private void ServerTryWeldingGrindingCollecting()
    {
        ...
        var ready = Welder.Enabled && Welder.IsWorking && Welder.IsFunctional;
        ...
        if (ready)
        {
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerTryWeldingGrindingCollecting Welder ready: Enabled={1}, IsWorking={2}, IsFunctional={3}",
                              Logging.BlockName(Welder, Logging.BlockNameOptions.None), Welder.Enabled, Welder.IsWorking, Welder.IsFunctional);
            ...
            {
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerTryWeldingGrindingCollecting TryWeldGrind: Mode {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Settings.WorkMode);
        ...
        else
        {
            ...
            State.MissingComponents.RebuildHash();
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: TryWelding Welder not ready: Enabled={1}, IsWorking={2}, IsFunctional={3}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                              Welder.Enabled || _CreativeModeActive, Welder.IsWorking, Welder.IsFunctional);
        }
        ...
        if (!(welding || grinding || collecting || transporting) && _TransportInventory.CurrentVolume > 0)
        {
            ...
            if (State.LastTransportTarget.HasValue)
            {
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: Idle but not empty started transporttime={1} CurrentVolume={2}/{3}",
                                  Logging.BlockName(Welder, Logging.BlockNameOptions.None), State.CurrentTransportTime, _TransportInventory.CurrentVolume, _MaxTransportVolume);
                transporting = true;
            }
        ...
        if (missingComponentsChanged && Mod.Log.ShouldLog(Logging.Level.Verbose))
            lock (Mod.Log)
            {
                Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: TryWelding: MissingComponents --->", Logging.BlockName(Welder, Logging.BlockNameOptions.None));
    ...
    private void ServerTryPushInventory()
    {
        ...
        _TryAutoPushInventoryLast = MyAPIGateway.Session.ElapsedPlayTime;

        var welder = (MyShipWelder)Welder;
        foreach (var item in welder.GetInventory().GetItems().ToArray())
        {
            if (Settings.Flags.HasFlag(SyncBlockSettings.Settings.PushIngotOreImmediately))
            {
                if (item.Content.TypeId == typeof(MyObjectBuilder_Ore) || item.Content.TypeId == typeof(MyObjectBuilder_Ingot))
                    MyGridConveyorSystem.ItemPushRequest(welder, welder.GetInventory(), item);
                continue;
            }

            if (Settings.Flags.HasFlag(SyncBlockSettings.Settings.PushComponentImmediately))
            {
            ...
            if (Settings.Flags.HasFlag(SyncBlockSettings.Settings.PushItemsImmediately))
            {
                MyGridConveyorSystem.ItemPushRequest(welder, welder.GetInventory(), item);
            }
        }
        ...
        if (welderInventory != null)
        {
            ...
            for (var srcItemIndex = tempInventoryItems.Count - 1; srcItemIndex >= 0; srcItemIndex--)
            {
                ...
                if (srcItem.Type.TypeId == typeof(MyObjectBuilder_Ore).Name || srcItem.Type.TypeId == typeof(MyObjectBuilder_Ingot).Name)
                {
                    if ((Settings.Flags & SyncBlockSettings.Settings.PushIngotOreImmediately) != 0)
                    {
                        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerTryPushInventory TryPush IngotOre: Item={1} Amount={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                          srcItem.ToString(), srcItem.Amount);
                        welderInventory.PushComponents(_PossibleSources, (IMyInventory destInventory, IMyInventory srcInventory, ref MyInventoryItem srcItemIn) => { return _Ignore4Ingot.Contains(destInventory); }, srcItemIndex,
                                                       srcItem);
                ...
                else if (srcItem.Type.TypeId == typeof(MyObjectBuilder_Component).Name)
                {
                    ...
                    {
                        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerTryPushInventory TryPush Component: Item={1} Amount={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                          srcItem.ToString(), srcItem.Amount);
                        welderInventory.PushComponents(_PossibleSources, (IMyInventory destInventory, IMyInventory srcInventory, ref MyInventoryItem srcItemIn) => { return _Ignore4Components.Contains(destInventory); }, srcItemIndex,
                                                       srcItem);
                        _TryAutoPushInventoryLast = lastPush;
                    }
                }
                else
                {
                    ...
                    {
                        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerTryPushInventory TryPush Items: Item={1} Amount={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                          srcItem.ToString(), srcItem.Amount);
                        welderInventory.PushComponents(_PossibleSources, (IMyInventory destInventory, IMyInventory srcInventory, ref MyInventoryItem srcItemIn) => { return _Ignore4Items.Contains(destInventory); }, srcItemIndex,
                                                       srcItem);
                        _TryAutoPushInventoryLast = lastPush;
                    }
                }
    ...
    private void ServerTryCollectingFloatingTargets(out bool collecting, out bool needcollecting, out bool transporting)
    {
        ...
        {
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerTryCollectingFloatingTargets PossibleFloatingTargets={1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
            ...
            foreach (var targetData in State.PossibleFloatingTargets)
                ...
                {
                    if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                        Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerTryCollectingFloatingTargets: {1} distance={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                      Logging.BlockName(targetData.Entity), targetData.Distance);
                    needcollecting = true;
    ...
    public bool IsShieldProtected(IMySlimBlock block)
    {
        try
        {
            if (block != null && Shield != null && Shield.IsReady)
            {
                ...
                if (block?.CubeGrid.EntityId == Welder.CubeGrid.EntityId)
                    return false;
    ...
    private void ServerTryGrinding(out bool grinding, out bool needgrinding, out bool transporting, out IMySlimBlock currentGrindingBlock)
    {
        ...
        {
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerTryGrinding PossibleGrindTargets={1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), State.PossibleGrindTargets.CurrentCount);
            ...
            foreach (var targetData in State.PossibleGrindTargets)
            {
                ...
                {
                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                        Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerTryGrinding: {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(targetData.Block));
                    needgrinding = true;
    ...
    private void ServerTryWelding(out bool welding, out bool needwelding, out bool transporting, out IMySlimBlock currentWeldingBlock)
    {
        ...
        {
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerTryWelding PossibleWeldTargets={1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), State.PossibleWeldTargets.CurrentCount);
            foreach (var targetData in State.PossibleWeldTargets)
            {
                ...
                if ((Settings.Flags & SyncBlockSettings.Settings.ScriptControlled) != 0 || !targetData.Ignore && Weldable(targetData))
                {
                    needwelding = true;
                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                        Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerTryWelding: {1} HasDeformation={2} (MaxDeformation={3}), IsFullIntegrity={4}, HasFatBlock={5}, IsProjected={6}",
                                      Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(targetData.Block), targetData.Block.HasDeformation, targetData.Block.MaxDeformation, targetData.Block.IsFullIntegrity,
                                      targetData.Block.FatBlock != null, targetData.Block.IsProjected());
    ...
    private bool IsTransportRunnning(TimeSpan playTime)
    {
        if (State.CurrentTransportStartTime > TimeSpan.Zero)
        {
            ...
            if (State.CurrentTransportIsPick)
                ...
                {
                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                        Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: IsTransportRunnning transport still running transport inventory not emtpy",
                                      Logging.BlockName(Welder, Logging.BlockNameOptions.None));
                    return true;
                }
            ...
            if (playTime.Subtract(State.CurrentTransportStartTime) < State.CurrentTransportTime)
            {
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: IsTransportRunnning: transport still running remaining transporttime={1}",
                                  Logging.BlockName(Welder, Logging.BlockNameOptions.None), State.CurrentTransportTime.Subtract(MyAPIGateway.Session.ElapsedPlayTime.Subtract(State.CurrentTransportStartTime)));
                return true;
            }
    ...
    public void TriggerTerminalRefresh()
    {
        ...
        if (Welder != null && MyAPIGateway.Gui.InteractedEntity == Welder)
        {
            var action = Welder.GetActionWithName("helpOthers");
            if (action != null)
            {
                action.Apply(Welder);
                action.Apply(Welder);
    ...
    private bool ServerDoWeld(TargetBlockData targetData)
    {
        ...
        if ((targetData.Attributes & TargetBlockData.AttributeFlags.Projected) != 0)
        {
            ...
            if ((_CreativeModeActive || item != null && item.Value.Amount >= 1) && cubeGridProjected?.Projector != null)
            {
                if (Welder.IsWithinWorldLimits(cubeGridProjected.Projector, blockDefinition.BlockPairName, blockDefinition.PCU))
                {
                    var blockBuildIntegrity = target.BuildIntegrity;

                    if (!cubeGridProjected.Projector.Closed && !cubeGridProjected.Projector.CubeGrid.Closed && (target.FatBlock is not { Closed: true } || !target.FatBlock.IsVisible()))
                        ((IMyProjector)cubeGridProjected.Projector).Build(target, Welder.OwnerId, Welder.EntityId, true, Welder.SlimBlock.BuiltBy);
                    ...
                    if (!_CreativeModeActive && item.HasValue)
                    {
                        var newIntegrity = target?.BuildIntegrity;
                        if (newIntegrity > blockBuildIntegrity)
                            _TransportInventory.RemoveItems(item.Value.ItemId, 1);
                    }

                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                        Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoWeld (new): {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(target));
                }
                else
                {
        ...
        if (!hasIgnoreColor && target != null && (targetData.Attributes & TargetBlockData.AttributeFlags.Projected) == 0)
        {
            ...
            if (!target.IsFullIntegrity || created)
            {
                ...
                {
                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                        Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoWeld (incomplete): {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(target));

                    target.IncreaseMountLevel(MyAPIGateway.Session.WelderSpeedMultiplier * NanobotBuildAndRepairSystemMod.Settings.Welder.WeldingMultiplier * WELDER_AMOUNT_PER_SECOND, Welder.OwnerId, welderInventory,
                                              MyAPIGateway.Session.WelderSpeedMultiplier * NanobotBuildAndRepairSystemMod.Settings.Welder.WeldingMultiplier * WELDER_MAX_REPAIR_BONE_MOVEMENT_SPEED, Welder.HelpOthers);
                }
            ...
            else
            {
                //Deformation
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoWeld (deformed): {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(target));
                welding = true;
                target.IncreaseMountLevel(MyAPIGateway.Session.WelderSpeedMultiplier * NanobotBuildAndRepairSystemMod.Settings.Welder.WeldingMultiplier * WELDER_AMOUNT_PER_SECOND, Welder.OwnerId, welderInventory,
                                          MyAPIGateway.Session.WelderSpeedMultiplier * NanobotBuildAndRepairSystemMod.Settings.Welder.WeldingMultiplier * WELDER_MAX_REPAIR_BONE_MOVEMENT_SPEED, Welder.HelpOthers);
            }
    ...
    private bool ServerDoGrind(TargetBlockData targetData, out bool transporting)
    {
        ...
        var isEmpty = false;
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerDoGrind {1} integrityRatio={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(target), integrityRatio);
        if (integrityRatio <= 0.2)
            ...
            if (target.FatBlock != null && target.FatBlock.HasInventory)
            {
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerDoGrind {1} Try empty Inventory running={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(target),
                                  emptying);
            }

        if (!emptying || isEmpty)
        {
            var damageInfo = new MyDamageInformation(false, damage, MyDamageType.Grind, Welder.EntityId);
            ...
            if (target.UseDamageSystem)
                ...
                {
                    var relation = entry.Value.Welder.GetUserRelationToOwner(Welder.OwnerId);
                    if (relation.IsFriendly())
                    {
                        ...
                        if (Mod.Log.ShouldLog(Logging.Level.Info))
                            Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock: Damaged Add FriendlyDamage {0} Timeout {1}", Logging.BlockName(target), entry.Value.FriendlyDamage[target]);
                    }
                }
            ...
            {
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoGrind {1} FullyDismounted", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(target));
                if (target.UseDamageSystem)
                {
                    //Not available in modding
        ...
        if ((float)_TransportInventory.CurrentVolume >= _MaxTransportVolume || target.IsFullyDismounted)
        {
            ...
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoGrind: Target {1} transport started transporttime={2} CurrentVolume={3}/{4}",
                              Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(targetData.Block), State.CurrentTransportTime, _TransportInventory.CurrentVolume, _MaxTransportVolume);
            ServerEmptyTranportInventory(true);
            transporting = true;
        }
    ...
    private bool ServerDoCollectFloating(TargetEntityData targetData, out bool transporting, ref TargetEntityData collectingFirstTarget)
    {
        ...
        if (targetData != null)
        {
            ...
            if (canAdd)
            {
                ...
                else
                {
                    ...
                    if (isEmpty)
                    {
                        if (target is IMyCharacter character && character.IsBot)
                            //Wolf, Spider, ...
                            target.Delete();
                    }
                }
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerDoCollectFloating {1} Try pick floating running={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                  Logging.BlockName(target), collecting);
        ...
        if (collectingFirstTarget != null && ((float)_TransportInventory.CurrentVolume >= _MaxTransportVolume || !canAdd && _TransportInventory.CurrentVolume > 0))
        {
            ...
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoCollectFloating: Target {1} transport started transporttime={2} CurrentVolume={3}/{4}",
                              Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(collectingFirstTarget.Entity), State.CurrentTransportTime, _TransportInventory.CurrentVolume, _MaxTransportVolume);
            ServerEmptyTranportInventory(true);
            transporting = true;
    ...
    private bool ServerFindMissingComponents(TargetBlockData targetData)
    {
        try
        {
                    ...
                    if (picked)
                        if ((Settings.Flags & SyncBlockSettings.Settings.UseIgnoreColor) == 0 || !IsColorNearlyEquals(Settings.IgnoreColorPacked, targetData.Block.GetColorMask()))
                        {
                            ...
                            targetData.Block.GetMissingComponents(_TempMissingComponents,
                                                                  (Settings.WeldOptions & AutoWeldOptions.FunctionalOnly) == 0 ? UtilsInventory.IntegrityLevel.Complete : UtilsInventory.IntegrityLevel.Functional);
            ...
            if (remainingVolume < _MaxTransportVolume || _CreativeModeActive && _TempMissingComponents.Count > 0)
            {
                ...
                State.CurrentTransportTime = TimeSpan.FromSeconds(2d * targetData.Distance / Settings.TransportSpeed);
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: FindMissingComponents: Target {1} transport started volume={2} (max {3}) transporttime={4}",
                                  Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(targetData.Block), _MaxTransportVolume - remainingVolume, _MaxTransportVolume, State.CurrentTransportTime);
                return true;
            }
    ...
    private bool ServerFindMissingComponents(TargetBlockData targetData, ref float remainingVolume)
    {
        ...
        {
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: FindMissingComponents: Target {1} missing {2}={3} remainingVolume={4}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                              Logging.BlockName(targetData.Block), keyValue.Key, keyValue.Value, remainingVolume);
            var neededAmount = 0;
    ...
    {
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerPickFromWelder Try: {1}={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId, neededAmount);
        ...
        {
            if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerPickFromWelder welder empty: {1}={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId, neededAmount);
            return picked;
        }
        ...
        for (var i1 = tempInventoryItems.Count - 1; i1 >= 0; i1--)
        {
            ...
            if (srcItem != null && (MyDefinitionId)srcItem.Type == componentId && srcItem.Amount > 0)
            {
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerPickFromWelder: {1}: missingAmount={2} pickedAmount={3} maxpossibleAmount={4} remainingVolume={5} transportVolumeTotal={6}",
                                  Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId, neededAmount, pickedAmount, maxpossibleAmount, remainingVolume, _TransportInventory.CurrentVolume);
            }
    ...
    /// <summary>
    ///     Check if the transport inventory is empty after delivering/grinding/collecting, if not move items back to welder
    ///     inventory
    /// </summary>
        ...
        if (!empty)
        {
            if (!_CreativeModeActive)
            {
                ...
                if (welderInventory != null)
                {
                    ...
                    {
                        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: ServerEmptyTranportInventory: push={1}: MaxVolume={2} CurrentVolume={3} Timeout={4}",
                                          Logging.BlockName(Welder, Logging.BlockNameOptions.None), push, welderInventory.MaxVolume, welderInventory.CurrentVolume,
                                          MyAPIGateway.Session.ElapsedPlayTime.Subtract(_TryPushInventoryLast).TotalSeconds);
                        if (MyAPIGateway.Session.ElapsedPlayTime.Subtract(_TryPushInventoryLast).TotalSeconds > 3 && welderInventory.MaxVolume - welderInventory.CurrentVolume < _TransportInventory.CurrentVolume * 1.5f)
                            if (!Welder.PushComponents(_PossibleSources, null))
                                //Failed retry after timeout
                                _TryPushInventoryLast = MyAPIGateway.Session.ElapsedPlayTime;
                    }

                    var list = _TransportInventory.GetItems();
                    for (var srcItemIndex = 0; srcItemIndex < list.Count; srcItemIndex++)
                    {
                        var item = list[srcItemIndex];
                        //Try to move as much as possible
                        var amount = item.Amount;
                        var moveableAmount = MyInventory.Transfer(_TransportInventory, welderInventory, (uint)srcItemIndex);
                        amount -= moveableAmount;
                        
                        if (moveableAmount > 0 && Mod.Log.ShouldLog(Logging.Level.Info))
                            Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerEmptyTranportInventory move to welder Item {1} amount={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), item.Content.GetObjectId(),
                                          moveableAmount);
                        if (amount > 0 && Mod.Log.ShouldLog(Logging.Level.Info))
                            Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerEmptyTranportInventory (no more room in welder) Item {1} amount={2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                          item.Content.GetObjectId(), amount);
                    }
                }
            }
            else
    ...
    private bool EmptyBlockInventories(IMyEntity entity, IMyInventory dstInventory, out bool isEmpty)
    {
        ...
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: EmptyBlockInventories remainingVolume={1} Entity={2}, InventoryCount={3}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), remainingVolume,
                          Logging.BlockName(entity, Logging.BlockNameOptions.None), entity.InventoryCount);
        ...
        for (var i1 = 0; i1 < entity.InventoryCount; i1++)
        {
            ...
            for (var srcItemIndex = tempInventoryItems.Count - 1; srcItemIndex >= 0; srcItemIndex--)
            {
                ...
                if (dstInventory.TransferItemFrom(srcInventory, srcItemIndex, null, true, maxpossibleAmount, false))
                {
                    ...
                    if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                        Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: EmptyBlockInventories Transfered Item {1} amount={2} remainingVolume={3}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                      srcItem.Content.GetId(), maxpossibleAmount, remainingVolume);
                    running = true;
    ...
    private bool EmptyFloatingObject(MyFloatingObject floating, IMyInventory dstInventory, out bool isEmpty)
    {
        ...
        var possibleAmount = ((MyInventory)dstInventory).ComputeAmountThatFits(floating.Item.Content.GetObjectId());
            
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: EmptyFloatingObject Item={1}, ItemAmount={2}, MaxPossibleAmount={3})",
                          Logging.BlockName(Welder, Logging.BlockNameOptions.None), floating.Item.Content.GetObjectId(), floating.Item.Amount, possibleAmount);
        
        if (possibleAmount == MyFixedPoint.Zero)
            return running;
            
        if (possibleAmount >= floating.Item.Amount)
        {
        ...
        {
            floating.Item.Amount -= possibleAmount;
            floating.RefreshDisplayName();
        }

        dstInventory.AddItems(possibleAmount, floating.Item.Content);
        
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: EmptyFloatingObject Removed Item {1} amount={2} remainingItemAmount={3}",
                          Logging.BlockName(Welder, Logging.BlockNameOptions.None), floating.Item.Content.GetId(), possibleAmount, floating.Item.Amount);
        running = true;
    ...
    {
        if (State.MissingComponents.TryGetValue(componentId, out var missingAmount))
    ...
    private bool PullComponents(MyDefinitionId componentId, float volume, ref int neededAmount, ref float remainingVolume)
    {
        ...
        var amount = ((MyCubeGrid)Welder.CubeGrid).GridSystems.ConveyorSystem.PullItem(componentId, maxPossibleAmount, Welder, Welder.GetInventory(), false);

        neededAmount -= (int)amount;
        remainingVolume -= (float)amount * volume;
        ...
        var maxpossibleAmount = Math.Min(neededAmount, (int)Math.Ceiling(remainingVolume / volume));
        if (Mod.Log.ShouldLog(Logging.Level.Info))
            Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: PullComponents start: {1}={2} maxpossibleAmount={3} volume={4}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId, neededAmount,
                          maxpossibleAmount, volume);
                ...
                if (srcInventory.FindItem(componentId) != null && srcInventory.CanTransferItemTo(welderInventory, componentId))
                {
                    ...
                    for (var srcItemIndex = tempInventoryItems.Count - 1; srcItemIndex >= 0; srcItemIndex--)
                    {
                        ...
                        if (srcItem != null && (MyDefinitionId)srcItem.Type == componentId && srcItem.Amount > 0)
                        {
                            ...
                            if (Mod.Log.ShouldLog(Logging.Level.Info))
                                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: PullComponents Found: {1}={2} in {3}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId, srcItem.Amount,
                                              Logging.BlockName(srcInventory));
                            ...
                            if (amountPossible > 0)
                            {
                                ...
                                {
                                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                                        Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: PullComponents Try to move: {1}={2} from {3}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId,
                                                      amountMoveable, Logging.BlockName(srcInventory));
                                    ...
                                    if (moved)
                                    {
                                        ...
                                        if (Mod.Log.ShouldLog(Logging.Level.Info))
                                            Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: PullComponents Moved: {1}={2} from {3}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId,
                                                          amountMoveable, Logging.BlockName(srcInventory));
                                ...
                                else
                                {
                                    ...
                                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                                        Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: PullComponents no more space in welder: {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), componentId);
    ...
    private void StartAsyncUpdateSourcesAndTargets(bool updateSource)
    {
        ...
        if (!Welder.Enabled || !Welder.IsFunctional || State.Ready == false)
        {
            if (Mod.Log.ShouldLog(Logging.Level.Info))
                Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: AsyncUpdateSourcesAndTargets Enabled={1} IsFunctional={2} ---> not ready don't search for targets",
                              Logging.BlockName(Welder, Logging.BlockNameOptions.None), Welder.Enabled, Welder.IsFunctional);
            lock (State.PossibleWeldTargets)
            {
    ...
    public void AsyncUpdateSourcesAndTargets(bool updateSource)
    {
        try
        {
            ...
            updateSource &= Welder.UseConveyorSystem;
            var pos = 0;
            try
            {
                pos = 1;
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: AsyncUpdateSourcesAndTargets Search: IgnoreColor={1}, GrindColor={2}, UseGrindJanitorOn={3}, Settings.WorkMode={4}, GrindJanitorOptions={5}",
                                  Logging.BlockName(Welder, Logging.BlockNameOptions.None), ignoreColor, grindColor, Settings.UseGrindJanitorOn, Settings.WorkMode, Settings.GrindJanitorOptions);

                AsyncAddBlocksOfGrid(ref areaOrientedBox, (Settings.Flags & SyncBlockSettings.Settings.UseIgnoreColor) != 0, ref ignoreColor, (Settings.Flags & SyncBlockSettings.Settings.UseGrindColor) != 0, ref grindColor,
                                     Settings.UseGrindJanitorOn, Settings.GrindJanitorOptions, (MyCubeGrid)Welder.CubeGrid, grids, updateSource ? _TempPossibleSources : null, weldingEnabled ? _TempPossibleWeldTargets : null,
                ...
                switch (Settings.SearchMode)
                {
                    ...
                    case SearchModes.BoundingBox:
                        AsyncAddBlocksOfBox(ref areaOrientedBox, (Settings.Flags & SyncBlockSettings.Settings.UseIgnoreColor) != 0, ref ignoreColor, (Settings.Flags & SyncBlockSettings.Settings.UseGrindColor) != 0, ref grindColor,
                                            Settings.UseGrindJanitorOn, Settings.GrindJanitorOptions, grids, weldingEnabled ? _TempPossibleWeldTargets : null, grindingEnabled ? _TempPossibleGrindTargets : null,
                                            ComponentCollectPriority.AnyEnabled ? _TempPossibleFloatingTargets : null);
                        break;
                }
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                {
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: AsyncUpdateSourcesAndTargets Possible Build Target Blocks ---> {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                      _TempPossibleWeldTargets.Count);
                    Mod.Log.IncreaseIndent(Logging.Level.Verbose);
                    ...
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: AsyncUpdateSourcesAndTargets Possible Grind Target Blocks ---> {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                  _TempPossibleGrindTargets.Count);
                    Mod.Log.IncreaseIndent(Logging.Level.Verbose);
                    ...
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: AsyncUpdateSourcesAndTargets Possible Floating Targets ---> {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                  _TempPossibleFloatingTargets.Count);
                    Mod.Log.IncreaseIndent(Logging.Level.Verbose);
                    ...
                    if (updateSource)
                    {
                        Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: AsyncUpdateSourcesAndTargets Possible Source Blocks ---> {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                                      _TempPossibleSources.Count);
                        Mod.Log.IncreaseIndent(Logging.Level.Verbose);
                        foreach (var inventory in _TempPossibleSources)
                            Mod.Log.Write(Logging.Level.Verbose, "Inventory: {0} {1}{2}{3}", Logging.BlockName(inventory), _TempIgnore4Ingot.Contains(inventory) ? string.Empty : "(Not 4 Ingot)",
                                          _TempIgnore4Components.Contains(inventory) ? string.Empty : "(Not 4 Components)", _TempIgnore4Items.Contains(inventory) ? string.Empty : "(Not 4 Items)");
                        Mod.Log.DecreaseIndent(Logging.Level.Verbose);
                        Mod.Log.Write(Logging.Level.Verbose, "<---");
                    }
                }
                ...
                lock (State.PossibleWeldTargets)
                {
                    ...
                    State.PossibleWeldTargets.Sort((a, b) =>
                    {
                ...
                lock (State.PossibleGrindTargets)
                {
                    ...
                    State.PossibleGrindTargets.Sort((a, b) =>
                    {
                        ...
                        if ((a.Attributes & TargetBlockData.AttributeFlags.Autogrind) != 0) return -1;
                        if ((b.Attributes & TargetBlockData.AttributeFlags.Autogrind) != 0) return 1;
                        return 0;
                    });
                    State.PossibleGrindTargets.RebuildHash();
                }
                ...
                lock (State.PossibleFloatingTargets)
                {
                    ...
                    State.PossibleFloatingTargets.Sort((a, b) =>
                    {
                        var itemA = a.Entity;
                        var itemB = b.Entity;
                        ...
                        if (itemAFloating == null) return -1;
                        if (itemBFloating == null) return 1;
                        return Utils.CompareDistance(a.Distance, b.Distance);
                    });
                    State.PossibleFloatingTargets.RebuildHash();
                }
                ...
                if (updateSource)
                {
                    ...
                    Welder.SlimBlock.ComputeWorldCenter(out var posWelder);
                    _PossibleSources.Sort((a, b) =>
                    {
                        ...
                        if (blockA != null && blockB != null)
                        {
                            ...
                            {
                                blockA.SlimBlock.ComputeWorldCenter(out var posA);
                                blockB.SlimBlock.ComputeWorldCenter(out _);
                    ...
                    foreach (var inventory in _PossibleSources)
                    {
                        if (inventory.Owner is not MyShipWelder { GameLogic: NanobotBuildAndRepairSystemBlock bar })
                            continue;
            ...
            catch (Exception ex)
            {
                ...
                {
                    Mod.Log.Error("BuildAndRepairSystemBlock {0}: AsyncUpdateSourcesAndTargets exception at {1}: {2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), pos, ex);
                    _ContinuouslyError = 0;
                }
            }
        }
    ...
    /// </summary>
    private void AsyncAddBlocksOfBox(ref MyOrientedBoundingBoxD areaBox, bool useIgnoreColor, ref uint ignoreColor, bool useGrindColor, ref uint grindColor, AutoGrindRelation autoGrindRelation, AutoGrindOptions autoGrindOptions,
                                     ISet<IMyCubeGrid> grids, ISet<TargetBlockData>? possibleWeldTargets, ISet<TargetBlockData>? possibleGrindTargets, ISet<TargetEntityData>? possibleFloatingTargets)
    {
        if (Mod.Log.ShouldLog(Logging.Level.Verbose)) Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: AsyncAddBlockOfBox", Logging.BlockName(Welder, Logging.BlockNameOptions.None));

        var emitterMatrix = Welder.WorldMatrix;
        emitterMatrix.Translation = Vector3D.Transform(Settings.CorrectedAreaOffset, emitterMatrix);
        var areaBoundingBox = Settings.CorrectedAreaBoundingBox.TransformFast(emitterMatrix);
        
        using var entityInRange = _tempEntities;
        MyGamePruningStructure.GetAllEntitiesInBox(ref areaBoundingBox, entityInRange);
            ...
            switch (entity)
            {
                case MyFloatingObject floating:
                {
                    if (!floating.MarkedForClose && ComponentCollectPriority.GetEnabled(floating.Item.Content.GetObjectId()))
                    {
                        ...
                        possibleFloatingTargets.Add(new(floating, distance));
                    }

                    continue;
                }
                case IMyCharacter character:
                {
                    if (character.IsDead && !character.InventoriesEmpty() && !((MyCharacterDefinition)character.Definition).EnableSpawnInventoryAsContainer)
                    {
                        ...
                        possibleFloatingTargets.Add(new(character, distance));
                    }

                    continue;
                }
                case IMyInventoryBag inventoryBag:
                {
                    if (!inventoryBag.InventoriesEmpty())
                    {
                        var distance = (areaBox.Center - inventoryBag.WorldMatrix.Translation).Length();
                        possibleFloatingTargets.Add(new(inventoryBag, distance));
                    }
    ...
    /// <summary>
    /// </summary>
    private void AsyncAddBlocksOfGrid(ref MyOrientedBoundingBoxD areaBox, bool useIgnoreColor, ref uint ignoreColor, bool useGrindColor, ref uint grindColor, AutoGrindRelation autoGrindRelation, AutoGrindOptions autoGrindOptions,
                                      MyCubeGrid cubeGrid, ISet<IMyCubeGrid> grids, ISet<IMyInventory>? possibleSources, ISet<TargetBlockData>? possibleWeldTargets, ISet<TargetBlockData>? possibleGrindTargets)
    {
        if (!State.Ready) return; //Block not ready
        if (grids.Contains(cubeGrid)) return; //Allready parsed

        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: AsyncAddBlocksOfGrid AddGrid {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), cubeGrid.DisplayName);
        grids.Add(cubeGrid);
        ...
        foreach (var slimBlock in cubeGrid.GetBlocks())
        {
            ...
            switch (fatBlock)
            {
                case null:
                    continue;
                case MyMechanicalConnectionBlockBase mechanicalConnectionBlock:
                {
                    if (mechanicalConnectionBlock.TopGrid is { } topCubeGrid)
                        AsyncAddBlocksOfGrid(ref areaBox, useIgnoreColor, ref ignoreColor, useGrindColor, ref grindColor, autoGrindRelation, autoGrindOptions, topCubeGrid, grids, possibleSources,
                                             possibleWeldTargets, possibleGrindTargets);
                    continue;
                }
                case MyAttachableTopBlockBase attachableTopBlock:
                {
                    if (attachableTopBlock.Stator?.CubeGrid is { } statorCubeGrid)
                        AsyncAddBlocksOfGrid(ref areaBox, useIgnoreColor, ref ignoreColor, useGrindColor, ref grindColor, autoGrindRelation, autoGrindOptions, statorCubeGrid, grids, possibleSources,
                                             possibleWeldTargets, possibleGrindTargets);
                    continue;
                }
                case MyShipConnector connector:
                {
                    if (connector is { Connected: true, Other.CubeGrid: { } otherCubeGrid })
                        AsyncAddBlocksOfGrid(ref areaBox, useIgnoreColor, ref ignoreColor, useGrindColor, ref grindColor, autoGrindRelation, autoGrindOptions, otherCubeGrid, grids, possibleSources,
                                             possibleWeldTargets, possibleGrindTargets);
                    continue;
                }
            }

            if (possibleWeldTargets == null || !Settings.Flags.HasFlag(SyncBlockSettings.Settings.AllowBuild) || fatBlock is not IMyProjector projector) //If projected blocks should be build
                continue;
            
            if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: Projector={1} IsProjecting={2} BuildableBlockCount={3} IsRelationAllowed={4} Relation={5}/{6}/{7}",
                              Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(projector), projector.IsProjecting, projector.BuildableBlocksCount, IsRelationAllowed4Welding(slimBlock),
                              slimBlock.GetUserRelationToOwner(Welder.OwnerId), projector.GetUserRelationToOwner(Welder.OwnerId), slimBlock.CubeGrid.GetUserRelationToOwner(Welder.OwnerId));
                    
            if (projector is not { IsProjecting: true, ProjectedGrid: MyCubeGrid projectedGrid } || !IsRelationAllowed4Welding(slimBlock) || 
                !grids.Add(projectedGrid))
                continue;
                    
            //Add buildable blocks
            foreach (var block in projectedGrid.GetBlocks().Concat(NanobotBuildAndRepairSystemMod.MultigridProjectorApi.Available ? 
                                                                       Enumerable.Range(0, NanobotBuildAndRepairSystemMod.MultigridProjectorApi.GetSubgridCount(projector.EntityId))
                                                                                 .SelectMany(b =>
                                                                                 {
                                                                                     var previewGrid = (MyCubeGrid)NanobotBuildAndRepairSystemMod.MultigridProjectorApi.GetPreviewGrid(projector.EntityId, b);

                                                                                     return grids.Add(previewGrid) ? previewGrid.GetBlocks() : Enumerable.Empty<MySlimBlock>();
                                                                                 }) : 
                                                                       Enumerable.Empty<MySlimBlock>()))
            {
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: Projector={1} Block={2} BlockKindEnabled={3}, InRange={4}, CanBuild={5}/{6} BlockClass={7}",
                                  Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(projector), Logging.BlockName(block), BlockWeldPriority.GetEnabled(block),
                                  block.IsInRange(ref areaBox, out distance), block.CanBuild(false), block.Dithering, BlockWeldPriority.GetItemAlias(block, true));
                    
                if (!BlockWeldPriority.GetEnabled(block) || !block.IsInRange(ref areaBox, out distance) || !block.CanBuild(false))
                    continue;
                    
                if (Mod.Log.ShouldLog(Logging.Level.Verbose))
                    Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: Add projected Block {1}:{2}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(projector),
                                  Logging.BlockName(block));
                    
                possibleWeldTargets.Add(new(block, distance, TargetBlockData.AttributeFlags.Projected));
            }
        }
    }
    ...
    /// <summary>
    /// </summary>
    private void AsyncAddBlockIfTargetOrSource(ref MyOrientedBoundingBoxD areaBox, bool useIgnoreColor, ref uint ignoreColor, bool useGrindColor, ref uint grindColor, AutoGrindRelation autoGrindRelation,
                                               AutoGrindOptions autoGrindOptions, IMySlimBlock block, ISet<IMyInventory>? possibleSources, ISet<TargetBlockData>? possibleWeldTargets, ISet<TargetBlockData>? possibleGrindTargets)
    {
        try
        {
            if (possibleSources != null)
            {
                ...
                if (block.FatBlock is IMyTerminalBlock terminalBlock && terminalBlock.EntityId != Welder.EntityId && terminalBlock.IsFunctional) //Own inventory is no external source (handled internally)
                {
                    var relation = terminalBlock.GetUserRelationToOwner(Welder.OwnerId);
                    if (relation.IsFriendly())
                        try
                        {
                            var welderInventory = Welder.GetInventory(0);
                        ...
                        {
                            Mod.Log.Write(Logging.Level.Event, "BuildAndRepairSystemBlock {0}: AsyncAddBlockIfTargetOrSource1 exception: {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), ex);
                        }
                }
            }
        ...
        {
            Mod.Log.Error("BuildAndRepairSystemBlock {0}: AsyncAddBlockIfTargetOrSource2 exception: {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), ex);
            throw;
        }
    }
    ...
    /// </summary>
    private bool AsyncAddBlockIfWeldTarget(ref MyOrientedBoundingBoxD areaBox, bool useIgnoreColor, ref uint ignoreColor, bool useGrindColor, ref uint grindColor, IMySlimBlock block, ISet<TargetBlockData> possibleWeldTargets)
    {
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose,
                          "BuildAndRepairSystemBlock {0}: Weld Check Block {1} IsProjected={2} IsDestroyed={3}, IsFullyDismounted={4}, HasFatBlock={5}, FatBlockClosed={6}, MaxDeformation={7}, (HasDeformation={8}), IsFullIntegrity={9}, Integrity={10}, NeedRepair={11}, Relation={12}, useIgnorColor={13}, HasIgnoreColor={14} ({15},{16})", //, ActionAllowed={17}",
                          Logging.BlockName(Welder, Logging.BlockNameOptions.None),
                          Logging.BlockName(block),
                          block.IsProjected(),
                          block.IsDestroyed, block.IsFullyDismounted, block.FatBlock != null, block.FatBlock != null ? block.FatBlock.Closed.ToString() : "-",
                          block.MaxDeformation, block.HasDeformation, block.IsFullIntegrity, block.Integrity, block.NeedRepair((Settings.WeldOptions & AutoWeldOptions.FunctionalOnly) != 0),
                          block.GetUserRelationToOwner(Welder.OwnerId),
        ...
        if (block.IsProjected(out var projector))
        {
            ...
            {
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: Add projected Block {1}, HasFatBlock={2}, Class={3}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(block),
                                  block.FatBlock != null, BlockWeldPriority.GetItemAlias(block, true));
                possibleWeldTargets.Add(new(block, distance, TargetBlockData.AttributeFlags.Projected));
                return true;
            }
        }
        else
        {
            ...
            {
                if (Mod.Log.ShouldLog(Logging.Level.Info))
                    Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: Add damaged Block {1} MaxDeformation={2}, (HasDeformation={3}), IsFullIntegrity={4}, HasFatBlock={5}",
                                  Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(block), block.MaxDeformation, block.HasDeformation, block.IsFullIntegrity, block.FatBlock != null);
                possibleWeldTargets.Add(new(block, distance, 0));
                return true;
            }
        }
    ...
    /// </summary>
    private bool AsyncAddBlockIfGrindTarget(ref MyOrientedBoundingBoxD areaBox, bool useGrindColor, ref uint grindColor, AutoGrindRelation autoGrindRelation, AutoGrindOptions autoGrindOptions, IMySlimBlock block,
                                            ISet<TargetBlockData> possibleGrindTargets)
    {
        ...
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
            Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: Grind Check Block {1} Projected={2} AutoGrindRelation={3} Relation={4} UseGrindColor={5} HasGrindColor={6} ({7},{8})/", // ActionAllowed={10}",
                          Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(block), block.IsProjected(), autoGrindRelation, block.GetUserRelationToOwner(Welder.OwnerId), useGrindColor,
        ...
        if (autoGrind)
        {
            var relation = block.GetUserRelationToOwner(Welder.OwnerId);
            autoGrind =
        ...
        {
            if (block.IsInRange(ref areaBox, out var distance))
            {
                ...
                if (Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: Add grind Block {1}", Logging.BlockName(Welder, Logging.BlockNameOptions.None), Logging.BlockName(block));
                possibleGrindTargets.Add(new(block, distance, autoGrind ? TargetBlockData.AttributeFlags.Autogrind : 0));
                return true;
            }
        }
    ...
    {
        var relation = Welder.OwnerId == 0 ? MyRelationsBetweenPlayerAndBlock.NoOwnership : block.GetUserRelationToOwner(Welder.OwnerId);
        if (relation == MyRelationsBetweenPlayerAndBlock.Enemies) return false;
        if (!Welder.HelpOthers && (relation == MyRelationsBetweenPlayerAndBlock.Neutral || relation == MyRelationsBetweenPlayerAndBlock.NoOwnership)) return false;
        return true;
    }
    ...
    private void AppendingCustomInfo(IMyTerminalBlock terminalBlock, StringBuilder customInfo)
    {
        ...
        customInfo.Append(Welder.SlimBlock.BlockDefinition.DisplayNameText);
        ...
        if (Welder.Enabled && Welder.IsWorking && Welder.IsFunctional)
        {
            if ((Settings.Flags & SyncBlockSettings.Settings.ScriptControlled) != 0)
            {
            ...
            lock (State.MissingComponents)
            {
                foreach (var component in State.MissingComponents)
                {
                    ...
                    var name = MyDefinitionManager.Static.TryGetComponentDefinition(componentId, out var componentDefnition) ? componentDefnition.DisplayNameText : component.Key.SubtypeName;
        ...
        {
            if (!Welder.Enabled) customInfo.Append(Texts.Info_BlockSwitchedOff + Environment.NewLine);
            else if (!Welder.IsFunctional) customInfo.Append(Texts.Info_BlockDamaged + Environment.NewLine);
            else if (!Welder.IsWorking) customInfo.Append(Texts.Info_BlockUnpowered + Environment.NewLine);
        }
    }
    ...
    private WorkingState GetWorkingState()
    {
        ...
        if (State.Welding) return WorkingState.Welding;
        if (State.NeedWelding)
        {
        ...
        if (State.Grinding) return WorkingState.Grinding;
        if (State.NeedGrinding)
        {
    ...
    private void SetWorkingEffects(WorkingState workingState)
    {
        ...
        switch (workingState)
        {
            ...
            case WorkingState.Grinding:
                     ...
                     workingState == WorkingState.Grinding && (NanobotBuildAndRepairSystemMod.Settings.Welder.AllowedEffects & VisualAndSoundEffects.GrindingVisualEffect) != 0))
                {
                    ...
                    MyParticlesManager.TryCreateParticleEffect(workingState == WorkingState.Welding ? PARTICLE_EFFECT_WELDING1 : PARTICLE_EFFECT_GRINDING1, ref MatrixD.Identity, ref Vector3D.Zero, uint.MaxValue,
                                                               out _ParticleEffectWorking1);
                    ...
                    if (flare != null)
                    {
                        ...
                        _LightEffect.Start(Vector3.Zero, new(0.7f, 0.85f, 1f, 1f), 5f, string.Concat(Welder.DisplayNameText, " EffectLight"));
                        ...
                        _LightEffect.GlareType = MyGlareTypeEnum.Normal;
                ...
                Welder.SetEmissiveParts("Emissive", Color.Red, 1.0f);
                Welder.SetEmissiveParts("EmissiveReady", Color.Green, 1.0f);
                Welder.SetEmissiveParts("EmissiveWorking", workingState == WorkingState.Welding ? Color.Yellow : Color.Blue, 1.0f);
                break;

            case WorkingState.MissingComponents:
                Welder.SetEmissiveParts("Emissive", Color.Red, 1.0f);
                Welder.SetEmissiveParts("EmissiveReady", Color.Red, 1.0f);
                Welder.SetEmissiveParts("EmissiveWorking", Color.Yellow, 1.0f);
                break;

            case WorkingState.InventoryFull:
                Welder.SetEmissiveParts("Emissive", Color.Red, 1.0f);
                Welder.SetEmissiveParts("EmissiveReady", Color.Red, 1.0f);
                Welder.SetEmissiveParts("EmissiveWorking", Color.Blue, 1.0f);
                break;
            ...
            case WorkingState.Idle:
                Welder.SetEmissiveParts("Emissive", Color.Red, 1.0f);
                Welder.SetEmissiveParts("EmissiveReady", Color.Green, 1.0f);
                Welder.SetEmissiveParts("EmissiveWorking", Color.Black, 1.0f);
                break;

            case WorkingState.Invalid:
            case WorkingState.NotReady:
                Welder.SetEmissiveParts("Emissive", Color.White, 1.0f);
                Welder.SetEmissiveParts("EmissiveReady", Color.Black, 1.0f);
                Welder.SetEmissiveParts("EmissiveWorking", Color.Black, 1.0f);
                break;
        }
        ...
        if (sound != null)
        {
            ...
            {
                _SoundEmitter = new((MyEntity)Welder);
            ...
            if (_SoundEmitterWorking == null)
            {
                _SoundEmitterWorking = new((MyEntity)Welder, true);
    ...
    private void UpdateWorkingEffectPosition(WorkingState workingState)
    {
        ...
        {
            State.CurrentWeldingBlock.GetWorldBoundingBox(out var box);
            matrix = box.Matrix;
            position = matrix.Translation;
        }
        else if (State.CurrentGrindingBlock != null)
        {
            State.CurrentGrindingBlock.GetWorldBoundingBox(out var box);
            matrix = box.Matrix;
            position = matrix.Translation;
        }
        else
        {
            matrix = Welder.WorldMatrix;
            position = matrix.Translation;
        }
    ...
    private void SetTransportEffects(bool active)
    {
        if ((NanobotBuildAndRepairSystemMod.Settings.Welder.AllowedEffects & VisualAndSoundEffects.TransportVisualEffect) != 0)
        {
            if (active)
            {
                ...
                {
                    MyParticlesManager.TryCreateParticleEffect(State.CurrentTransportIsPick ? PARTICLE_EFFECT_TRANSPORT1_PICK : PARTICLE_EFFECT_TRANSPORT1_DELIVER, ref MatrixD.Identity, ref Vector3D.Zero, uint.MaxValue,
                                                               out _ParticleEffectTransport1);
                    if (_ParticleEffectTransport1 != null)
                    {
    ...
    private void UpdateTransportEffectPosition()
    {
        ...
        startMatrix = Welder.WorldMatrix;
        startMatrix.Translation = Vector3D.Transform(_EmitterPosition, Welder.WorldMatrix);
    ...
    internal Vector3D? ComputePosition(object target)
    {
        ...
        {
            ((IMySlimBlock)target).ComputeWorldCenter(out var endPosition);
            return endPosition;
        }

        if (target is IMyEntity) return ((IMyEntity)target).WorldMatrix.Translation;
        if (target is Vector3D) return (Vector3D)target;
        return null;
    }
    ...
    /// </summary>
    /// <returns></returns>
    internal Dictionary<MyDefinitionId, int> GetMissingComponentsDict()
    {
        var dict = new Dictionary<MyDefinitionId, int>();
        lock (State.MissingComponents)
        {
