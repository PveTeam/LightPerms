...
public static class NanobotBuildAndRepairSystemTerminal
{
    ...
    public static bool CustomControlsInit;
    private static readonly List<IMyTerminalControl> CustomControls = [];
    ...
    public static void InitializeControls()
    {
        lock (CustomControls)
        {
            ...
            try
            {
                ...
                MyAPIGateway.TerminalControls.GetControls<IMyShipWelder>(out List<IMyTerminalControl> controls);
                ...
                CustomControls.Add(label);
                {
                    ...
                    comboBox.ComboBoxContent = list =>
                    {
                        if (NanobotBuildAndRepairSystemMod.Settings.Welder.AllowedSearchModes.HasFlag(SearchModes.Grids))
                            list.Add(new()
                                         { Key = (long)SearchModes.Grids, Value = Texts.SearchMode_Walk });
                        if (NanobotBuildAndRepairSystemMod.Settings.Welder.AllowedSearchModes.HasFlag(SearchModes.BoundingBox))
                            list.Add(new()
                                         { Key = (long)SearchModes.BoundingBox, Value = Texts.SearchMode_Fly });
                    };
                    comboBox.Getter = block =>
                    {
                        var system = GetSystem(block);
                        if (system == null) return 0;
                        return (long)system.Settings.SearchMode;
                    };
                    ...
                    foreach (var entry in list1)
                    {
                        ...
                        action.Name = new($"{entry.Value} On");
                        ...
                        checkbox.Setter = (block, value) =>
                        {
                            ...
                            if (system != null && !NanobotBuildAndRepairSystemMod.Settings.Welder.UseIgnoreColorFixed && isWeldingAllowed(block))
                            {
                                ...
                                foreach (var ctrl in CustomControls)
                                    if (ctrl.Id.Contains("IgnoreColor"))
                                        ctrl.UpdateVisual();
                            }
                        };
                        ...
                        checkbox.Setter = (block, value) =>
                        {
                            ...
                            if (system != null && isWeldingAllowed(block))
                            {
                                if (value)
                                {
                                    ...
                                    foreach (var ctrl in CustomControls)
                                        if (ctrl.Id.Contains("WeldOption"))
                                            ctrl.UpdateVisual();
                                }
                                else
                                    system.Settings.WeldOptions = system.Settings.WeldOptions & ~AutoWeldOptions.FunctionalOnly;
                    ...
                    //CustomControls.Add(separateArea);
                    {
                        ...
                        checkbox.Setter = (block, value) =>
                        {
                            ...
                            if (system != null && !NanobotBuildAndRepairSystemMod.Settings.Welder.UseGrindColorFixed && isGrindingAllowed(block))
                            {
                                ...
                                foreach (var ctrl in CustomControls)
                                    if (ctrl.Id.Contains("GrindColor"))
                                        ctrl.UpdateVisual();
                            }
                        };
                        checkbox.SupportsMultipleBlocks = true;
                        ...
                        checkbox.Setter = (block, value) =>
                        {
                            ...
                            if (system != null && !NanobotBuildAndRepairSystemMod.Settings.Welder.UseGrindJanitorFixed && isJanitorAllowed(block))
                            {
                                ...
                                if (value)
                                {
                                    ...
                                    foreach (var ctrl in CustomControls)
                                        if (ctrl.Id.Contains("GrindJanitorOption"))
                                            ctrl.UpdateVisual();
                                }
                                else
                ...
                CustomControls.Add(separateArea);
                {
                    ...
                    slider.Setter = (block, val) =>
                    {
                        ...
                        if (system != null)
                        {
                            ...
                            system.Settings.AreaOffset = new(val, system.Settings.AreaOffset.Y, system.Settings.AreaOffset.Z);
                        }
                    };
                    slider.Writer = (block, val) =>
                    {
                    ...
                    slider.Setter = (block, val) =>
                    {
                        ...
                        if (system != null)
                        {
                            ...
                            val = (float)Math.Round(val * 2, MidpointRounding.AwayFromZero) / 2f;
                            val = val < min ? min : val > max ? max : val;
                            system.Settings.AreaOffset = new(system.Settings.AreaOffset.X, val, system.Settings.AreaOffset.Z);
                        }
                    };
                    slider.Writer = (block, val) =>
                    {
                    ...
                    slider.Setter = (block, val) =>
                    {
                        ...
                        if (system != null)
                        {
                            ...
                            val = (float)Math.Round(val * 2, MidpointRounding.AwayFromZero) / 2f;
                            val = val < min ? min : val > max ? max : val;
                            system.Settings.AreaOffset = new(system.Settings.AreaOffset.X, system.Settings.AreaOffset.Y, val);
                        }
                    };
                    slider.Writer = (block, val) =>
                    {
                    ...
                    slider.Setter = (block, val) =>
                    {
                        ...
                        if (system != null)
                        {
                            ...
                            val = val < min ? min : val > max ? max : val;
                            system.Settings.AreaSize = new((int)Math.Round(val), system.Settings.AreaSize.Y, system.Settings.AreaSize.Z);
                        }
                    };
                    slider.Writer = (block, val) =>
                    {
                    ...
                    slider.Setter = (block, val) =>
                    {
                        ...
                        if (system != null)
                        {
                            ...
                            var max = getLimitMax(block);
                            val = val < min ? min : val > max ? max : val;
                            system.Settings.AreaSize = new(system.Settings.AreaSize.X, (int)Math.Round(val), system.Settings.AreaSize.Z);
                        }
                    };
                    slider.Writer = (block, val) =>
                    {
                    ...
                    slider.Setter = (block, val) =>
                    {
                        ...
                        if (system != null)
                        {
                            ...
                            var max = getLimitMax(block);
                            val = val < min ? min : val > max ? max : val;
                            system.Settings.AreaSize = new(system.Settings.AreaSize.X, system.Settings.AreaSize.Y, (int)Math.Round(val));
                        }
                    };
                    slider.Writer = (block, val) =>
                    {
                ...
                if (!NanobotBuildAndRepairSystemMod.Settings.Welder.ScriptControllFixed)
                {
                    ...
                    //Working Lists
                    var propertyMissingComponentsDict = MyAPIGateway.TerminalControls.CreateProperty<Dictionary<MyDefinitionId, int>, IMyShipWelder>("BuildAndRepair.MissingComponents");
                    ...
                    var propertyPossibleWeldTargetsList = MyAPIGateway.TerminalControls.CreateProperty<List<IMySlimBlock>, IMyShipWelder>("BuildAndRepair.PossibleTargets");
                    ...
                    var propertyPossibleGrindTargetsList = MyAPIGateway.TerminalControls.CreateProperty<List<IMySlimBlock>, IMyShipWelder>("BuildAndRepair.PossibleGrindTargets");
                    ...
                    var propertyPossibleCollectTargetsList = MyAPIGateway.TerminalControls.CreateProperty<List<IMyEntity>, IMyShipWelder>("BuildAndRepair.PossibleCollectTargets");
                    ...
                    //Control welding
                    var propertyCPT = MyAPIGateway.TerminalControls.CreateProperty<IMySlimBlock, IMyShipWelder>("BuildAndRepair.CurrentPickedTarget");
                    ...
                    propertyCPT.Setter = (block, value) =>
                    {
                        var system = GetSystem(block);
                        if (system != null)
                            system.Settings.CurrentPickedWeldingBlock = (VRage.Game.ModAPI.IMySlimBlock)value;
                    };
                    MyAPIGateway.TerminalControls.AddControl<IMyShipWelder>(propertyCPT);

                    var propertyCT = MyAPIGateway.TerminalControls.CreateProperty<IMySlimBlock, IMyShipWelder>("BuildAndRepair.CurrentTarget");
                    ...
                    //Control grinding
                    var propertyCPGT = MyAPIGateway.TerminalControls.CreateProperty<IMySlimBlock, IMyShipWelder>("BuildAndRepair.CurrentPickedGrindTarget");
                    ...
                    propertyCPGT.Setter = (block, value) =>
                    {
                        var system = GetSystem(block);
                        if (system != null)
                            system.Settings.CurrentPickedGrindingBlock = (VRage.Game.ModAPI.IMySlimBlock)value;
                    };
                    MyAPIGateway.TerminalControls.AddControl<IMyShipWelder>(propertyCPGT);

                    var propertyCGT = MyAPIGateway.TerminalControls.CreateProperty<IMySlimBlock, IMyShipWelder>("BuildAndRepair.CurrentGrindTarget");
                    ...
                    var propertyPEQ = MyAPIGateway.TerminalControls.CreateProperty<Func<IEnumerable<long>, MyDefinitionId, int, int>, IMyShipWelder>("BuildAndRepair.ProductionBlock.EnsureQueued");
                    ...
                    var propertyNC4B = MyAPIGateway.TerminalControls.CreateProperty<Func<IMyProjector, Dictionary<MyDefinitionId, MyFixedPoint>, int>, IMyShipWelder>("BuildAndRepair.Inventory.NeededComponents4Blueprint");
    ...
    {
        var action = MyAPIGateway.TerminalControls.CreateAction<IMyShipWelder>($"{name}_OnOff");
        action.Name = new($"{name} {onoffSwitch.OnText}/{onoffSwitch.OffText}");
        action.Icon = @"Textures\GUI\Icons\Actions\Toggle.dds";
        ...
        action = MyAPIGateway.TerminalControls.CreateAction<IMyShipWelder>($"{name}_On");
        action.Name = new($"{name} {onoffSwitch.OnText}");
        action.Icon = @"Textures\GUI\Icons\Actions\SwitchOn.dds";
        action.Enabled = onoffSwitch.Enabled;
        action.Action = block =>
        {
        ...
        action = MyAPIGateway.TerminalControls.CreateAction<IMyShipWelder>($"{name}_Off");
        action.Name = new($"{name} {onoffSwitch.OffText}");
        action.Icon = @"Textures\GUI\Icons\Actions\SwitchOff.dds";
        action.Enabled = onoffSwitch.Enabled;
        action.Action = block =>
        {
    ...
    private static void CreateSliderActions(string sliderName, IMyTerminalControlSlider slider)
    {
        ...
        action.Name = new($"{sliderName} Increase");
        ...
        action.Name = new($"{sliderName} Decrease");
