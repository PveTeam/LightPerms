...
public abstract class PriorityHandling<C, I> : List<PrioItemState<C>> where C : PrioItem //where C : struct
{
    ...
    private readonly Dictionary<int, int> _PrioHash = new();
    ...
    {
        get { return this.Any(i => i.Enabled); }
    }
    ...
    {
        var item = Find(i => i.PrioItem.Key == key);
    ...
    internal void FillTerminalList(List<MyTerminalControlListBoxItem> items, List<MyTerminalControlListBoxItem> selected)
    {
        foreach (var entry in this)
            if (entry.Visible)
            {
                var item = new MyTerminalControlListBoxItem(MyStringId.GetOrCompute(
                                                                $"({(entry.Enabled ? "X" : "-")}) {entry.PrioItem}"), MyStringId.NullOrEmpty, entry.PrioItem);
                items.Add(item);
    ...
    internal string GetEntries()
    {
        ...
        foreach (var entry in this)
            value += $"{entry.PrioItem.Key};{entry.Enabled}|";
    ...
    internal void SetEntries(string value)
    {
        ...
        foreach (var val in entries)
        {
            ...
            if (values.Length >= 2 &&
                int.TryParse(values[0], out var prioItemKey) &&
                bool.TryParse(values[1], out var enabled))
            {
        ...
        lock (_ClassList)
        {
            if (_HashDirty) //Second check now thread safe
            {
                ...
                foreach (var item in this)
                    _ClassList.Add($"{item.PrioItem.Key};{item.Enabled}");
                _PrioHash.Clear();
                var prio = 1;
                foreach (var item in this)
                {
